...
class Barang extends MY_Admin {
    ...
    public function ajax_delete($id)
  {
    ...
    public function getDetailBarang($id){
      $data['detail_data'] = $this->M_barang->where(['a.fc_kdstock' => $id])->row();
      // $data['varian'] = $this->M_barang->where_harga($id)->result();
      // $data['detail_qty'] = $this->M_barang->detail_qty_barang($id);
      // $data['campuran'] = $this->M_barang->harga_campuran($id)->result_array();

      // foreach ($data['varian'] as $key => $value) {
      //   $value->stok = $this->M_barang->getStok($value->id_varian_harga);
      // }
      echo json_encode($data); 
    }
