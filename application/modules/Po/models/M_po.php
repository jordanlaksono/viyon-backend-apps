...
class M_po extends CI_Model {
	...
	function where_max(){
		return $this->db->query('SELECT fc_nopo AS maxs FROM tm_po order by fc_nopo desc limit 1 ');
    }

    function getSupplier(){
    	return $this->db->get('tm_supplier');
    }

    function insert_po($where){
		$this->db->insert('tm_po',$where);
		return $this->db->insert_id();
	}

	function insert_detail_po($where){
		$this->db->insert('td_po',$where);
		return $this->db->insert_id();
	}
